...
def summary(
    ...
    aggregation: Union[AggregationType,
                       str] = AggregationType.mean) -> jax.Array:
  """Create a summary.
...
def sample_value(key: PRNGKey, val: jax.Array) -> jax.Array:
  i = jax.random.randint(key, [], 0, val.size, jnp.int32)
  return val.ravel()[i]


def aggregate_metric_list(
    metric_list: Sequence[Mapping[str, jax.Array]],
    use_jnp=False,
    key=None,
) -> MutableMapping[str, jax.Array]:
...
def _nested_to_names(ss: Any) -> Sequence[Tuple[str, jax.Array]]:
  """Attempt to parse out meaningful names and values from a pytree."""
  if isinstance(ss, jax.Array):
    return [("params", ss)]
