...
def tree_mean_abs(val):
  num_entry = sum(map(lambda x: onp.prod(x.shape), jax.tree_util.tree_leaves(val)))
  sum_abs = sum(map(lambda x: jnp.sum(jnp.abs(x)), jax.tree_util.tree_leaves(val)))
  return sum_abs / num_entry


@jax.jit
def tree_norm(val):
  sum_squared = sum(map(lambda x: jnp.sum(jnp.square(x)), jax.tree_util.tree_leaves(val)))
  return jnp.sqrt(sum_squared)

def tree_inner_product(tree_a, tree_b):
  # here we assume tree_a and tree_b have the same shape
  return sum(jax.tree_util.tree_map(
                              lambda a, b: jnp.sum(jnp.multiply(a, b)),
...
def tree_zip_onp(xs):
  ...
  ys = map(onp.asarray, zip(*map(lambda x: jax.tree_util.tree_flatten(x)[0], xs)))
...
def tree_dim(a):
  return sum(
    jax.tree_util.tree_leaves(
      jax.tree_util.tree_map(lambda x: np.prod(jnp.shape(x)), a)
      )
    )
  ...
  for l in leaves:
    if not hasattr(l, "dtype"):
      raise ValueError("The target struct doesn't have dtype specified?"
                       f" Value found: {l}")
  return jax.tree_util.tree_map(lambda a, b: jnp.asarray(a, dtype=b.dtype), struct1,
                           struct2)
...
def partition_unflatten(unflattener: PartitionUnflatten,
                        part_values: Sequence[jax.Array]) -> Any:
