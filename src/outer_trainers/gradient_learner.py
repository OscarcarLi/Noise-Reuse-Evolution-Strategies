...
class OptCheckpoint:
  ...
  elapsed_time: Union[float, jax.Array]
...
class GradientLearner:
  ...
  def init(self, key: PRNGKey) -> GradientLearnerState:
    ...
    if self._init_theta_from_path:
      ...
      real_param_checkpoint = checkpoints.load_state(
    ...
    if self._init_outer_state_from_path:
      ...
      real_checkpoint = checkpoints.load_state(
                                      self._init_outer_state_from_path,
                                      fake_checkpoint)
  ...
  ) -> Tuple[GradientLearnerState, Mapping[str, jax.Array]]:
...
class GradientEstimator(abc.ABC):
  ...
  def grad_est_name(self):
    return "Unimplemented GradientEstimator"
  ...
  ) -> Tuple[GradientEstimatorOut, Mapping[str, jax.Array]]:
    raise NotImplementedError()
...
def gradient_worker_compute(
  ...
  theta_shape = jax.tree_util.tree_map(lambda x: jax.ShapedArray(x.shape, x.dtype), theta)
  ...
  for si, (estimator,
    ...
    with profile.Profile(f"estimator{si}"):
      ...
      if estimator_out.unroll_info:
        ...
        onp_task_params = jax.tree_util.tree_map(onp.asarray,
        ...
        event_info.append({
            ...
            "iteration": iteration,
            "outer_iteration": worker_weights.outer_state.outer_iteration,  # type: ignore
        })
      else:
        logging.warn(
                "No out specified by learner. "
      ...
      metrics[f"sample||{family_name}/time"] = time.time() - stime  # type: ignore
  ...
  metrics["mean||block_time"] = block_time  # type: ignore
...
class SingleMachineGradientLearner:
  ...
  def init(self, theta_init_key: PRNGKey, grad_est_init_key: PRNGKey) -> SingleMachineState:
    """Initial state.
    ...
    theta_state = self.gradient_learner.init(theta_init_key)
    worker_weights = self.gradient_learner.get_state_for_worker(theta_state)
    keys = jax.random.split(grad_est_init_key, len(self.gradient_estimators))
    import tqdm
    print("Initializing gradient estimators")
    unroll_states = []
    for i, grad_est in tqdm.tqdm(enumerate(self.gradient_estimators)):
      key = keys[i]
      state = grad_est.init_worker_state(worker_weights, key)
      unroll_states.append(state)
      # if i % 25 == 0:
        # jax.clear_backends()
  ...
  def update(
      ...
      key: PRNGKey,
      with_metrics: bool = False
  ) -> Tuple[SingleMachineState, jax.Array, Mapping[str, jax.Array]]:
