...
def _sin_embedding(iteration: jax.Array) -> jax.Array:
...
class _LossNormalizer:
  ...
  def next_state(self, state: _LossNormalizerState,
                 loss: jax.Array) -> _LossNormalizerState:
  ...
  def corrected_mean(self, state: _LossNormalizerState) -> jax.Array:
    c = 1. / (1 - self.decay**jnp.asarray(state.updates, jnp.float32) + 1e-8)
    return state.mean * c


def _avg_square_mean(tree: Any) -> jax.Array:
  return sum([jnp.mean(jnp.square(x)) for x in jax.tree_leaves(tree)]) / len(
      jax.tree_leaves(tree))


def _clip_log_abs(value: jax.Array) -> jax.Array:
...
def _unstack(a: jax.Array, axis: int = 0) -> Sequence[jax.Array]:
  ...
  def opt_fn(self,
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def lstm_features_for_tensor(
          self, ms: jax.Array, rms: jax.Array, g: jax.Array,
          v: jax.Array, from_mlp: jax.Array, from_lstm: jax.Array,
          train_loss_feat: jax.Array,
          valid_loss_feat: jax.Array) -> Sequence[jax.Array]:
      ...
      def mlp_features_for_tensor(self, m: jax.Array, rms: jax.Array,
                                  g: jax.Array, v: jax.Array,
                                  ff_inputs: jax.Array,
                                  training_step: jax.Array,
                                  num_tensors: jax.Array) -> jax.Array:
      ...
      def update_is_training(self, opt_state, grads, loss,
        ...
        # Prep the features
        ff_feats = jax.tree_util.tree_map(to_map_get_mlp_features, ms, rms, grads,
