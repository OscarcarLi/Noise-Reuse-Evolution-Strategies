...
def rolling_mom(decay: float) -> _InitUpdate:
  ...
  def update_fn(state: MomAccumulator, grad: Any) -> MomAccumulator:
    m = jax.tree_util.tree_map(lambda a, b: decay * a + (1 - decay) * b, state.m,
                          grad)
...
def vec_factored_rolling(decays: jax.Array) -> _InitUpdate:
